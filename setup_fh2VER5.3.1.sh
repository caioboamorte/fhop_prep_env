...
hold_docker_packages() {
  ...
  log "Bloqueando versoes do Docker"
  ...
  if [[ "${#installed_packages[@]}" -eq 0 ]]; then
    warn "Nenhum pacote Docker instalado via dpkg foi encontrado para bloqueio."
    DOCKER_HOLD_STATUS="Nenhum pacote encontrado"
    return 1
  fi

  run_sudo apt-mark hold "${installed_packages[@]}"

  for pkg in "${installed_packages[@]}"; do
    if ! apt-mark showhold | grep -Fxq "$pkg"; then
      fail "Nao foi possivel confirmar o bloqueio do pacote $pkg."
      DOCKER_HOLD_STATUS="Falha na confirmacao"
      return 1
    fi
  done

  echo "Pacotes Docker bloqueados contra atualizacoes automaticas:"
  printf '  - %s\n' "${installed_packages[@]}"
  DOCKER_HOLD_STATUS="Ativo (${#installed_packages[@]} pacotes)"
}
...
Nao prossiga com a instalacao do FH2 OP ate que o Docker 27 esteja corretamente instalado. Versoes incompatíveis, especialmente a 29, podem causar erros fatais no frontend."
  ...
  if check_command docker && docker compose version >/dev/null 2>&1; then
    ...
    docker-compose --version || true
    installed_compose_version="$(docker-compose --version 2>/dev/null | sed -n 's/.*version[[:space:]]*v\?\([0-9][0-9.]*\).*/\1/p')"
  fi
  ...
  if [[ "$installed_docker_version" != "$EXPECTED_DOCKER_VERSION" || \
    ...
    DOCKER_STATUS="Versoes instaladas divergentes do pacote homologado"
    fail "As versoes instaladas nao correspondem integralmente ao pacote homologado.
Esperado: Docker $EXPECTED_DOCKER_VERSION, Compose $EXPECTED_COMPOSE_VERSION e containerd $EXPECTED_CONTAINERD_VERSION.
Detectado: Docker ${installed_docker_version:-desconhecido}, Compose ${installed_compose_version:-desconhecido} e containerd ${installed_containerd_version:-desconhecido}.
O bloqueio de versoes nao sera aplicado enquanto essa divergencia existir."
    return 1
  fi

  if ! hold_docker_packages; then
    DOCKER_STATUS="Docker correto, mas sem bloqueio confirmado"
    fail "Nao foi possivel bloquear todos os pacotes Docker instalados."
    return 1
  fi
